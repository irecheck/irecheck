...
class DecisionMaker():
    ...
    def dynamicoCallback(self, data):
        ...
        if (df.at[0, 'type'] == 'assessment'):
            if (min([df.at[0, 'pressureScore'],df.at[0, 'staticScore'],df.at[0, 'kinematicsScore'],df.at[0, 'tiltScore']]) == df.at[0, 'pressureScore']):
                msg = 'Jouons au jeu submarine!'
                rospy.loginfo(msg)
                self.pubSayMsg.publish(msg)
            if (min([df.at[0, 'pressureScore'],df.at[0, 'staticScore'],df.at[0, 'kinematicsScore'],df.at[0, 'tiltScore']]) == df.at[0, 'staticScore']):
                msg = 'Jouons au jeu chemist!'
                rospy.loginfo(msg)
                self.pubSayMsg.publish(msg)
            if (min([df.at[0, 'pressureScore'],df.at[0, 'staticScore'],df.at[0, 'kinematicsScore'],df.at[0, 'tiltScore']]) == df.at[0, 'kinematicsScore']):
                msg = 'Jouons au jeu pursuit!'
                rospy.loginfo(msg)
                self.pubSayMsg.publish(msg)
            if (min([df.at[0, 'pressureScore'],df.at[0, 'staticScore'],df.at[0, 'kinematicsScore'],df.at[0, 'tiltScore']]) == df.at[0, 'tiltScore']):
